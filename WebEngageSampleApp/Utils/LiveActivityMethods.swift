...
class LiveActivityMethods : NSObject {
    ...
    func startDeliveryOrder(controller: UIViewController) {
        if !(ActivityAuthorizationInfo().areActivitiesEnabled) {
            ...
            let OrderDeliveryAttributes = LiveActivityAttributes(numberOfPizzas: 1, totalAmount:"$99")
            
            let initialContentState = LiveActivityAttributes.LiveActivityStatus(driverName: "Uday", estimatedDeliveryTime:  Date()...Date().addingTimeInterval(1 * 60))
